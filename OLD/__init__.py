...
def least_squares(In, Qn):
    # This funcion requieres a In array and Qn Matix Exameple:
    #In = np.array([I0, I1, I2])
    #Qn = np.matrix([Q0, Q1, Q2])
    A = (Qn.T*Qn).I
    A = np.array(A)
    Qn = np.array(Qn)
    B = np.matmul(In.T, Qn)
...
def calculate_phase_rms_error(original_matrix, recovered_matrix):
    """
    Calcula el error RMS para matrices de fase, manejando la ambigüedad de 2*pi.
    """
    if original_matrix.shape != recovered_matrix.shape:
        raise ValueError(
            "Las dimensiones de las matrices deben ser idénticas.")
...
def calculate_rms_error(original_matrix, recovered_matrix):
    """
    Calcula el error RMS para matrices de amplitud.
    """
    #if original_matrix.shape != recovered_matrix.shape:
    #    raise ValueError(
    #        "Las dimensiones de las matrices deben ser idénticas.")
